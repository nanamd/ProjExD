...
def move_starship(scrn, joy_lr, joy_ud, jbtn1, jbtn2): # 自機の移動
...
def bring_enemy(): # 敵を出す
    ...
    if 150 < sec and sec < 175: # 150～175秒 ２種類
        if tmr%20 == 0:
            set_enemy(random.randint(20, 940), LINE_B, 270, EMY_ZAKO, 8, 1) # 敵1 下から上に
...
def main(): # メインループ
    ...
    while True:
        ...
        try:
            ...
            jbtn1 = joystick.get_button(0)+joystick.get_button(1)
            jbtn2 = joystick.get_button(2)+joystick.get_button(3)
        except:
            pass
        ...
        if idx == 0: # タイトル
            ...
            draw_text(screen, "Press Joy Button to start!", 480, 600, 50, SILVER)
            if jbtn1 != 0:
                idx = 1
                tmr = 0
                score = 0
        ...
        if idx == 1: # ゲームプレイ中
            move_starship(screen, joy_lr, joy_ud, jbtn1, jbtn2)
            move_missile(screen)
            bring_enemy()
            move_enemy(screen)
